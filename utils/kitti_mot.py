...
class kitti_mot(object):
    """Load and parse kitti mot data into a usable format."""

    def __init__(self, root_dir, split="0006", args=None):
        ...
        self.image_dir = os.path.join(self.root_dir, "image_02", split)
        self.lidar_dir = os.path.join(self.root_dir, "velodyne", split)
        self.label_filename = os.path.join(self.root_dir, "label_02", split + ".txt")
        self.calib_filename = os.path.join(self.root_dir, "calib", split + ".txt")
        self.oxts_filename = os.path.join(self.root_dir, "oxts", split + ".txt")

        self.num_samples = len(glob.glob(os.path.join(self.image_dir, '*.png')))
    ...
    def get_calibration(self):
        calib_filename = self.calib_filename
        return utils.Calibration(calib_filename, kitti_format='kitti-mot')

    def get_label_objects(self, idx):
        assert idx < self.num_samples
        label_filename = self.label_filename
        return utils.read_mot_label(label_filename, idx)
...
def show_image_with_boxes(img, objects, calib, show3d=True, depth=None, save_dir='output'):
    ...
    if save_dir is not None: # 将可视化结果保存为图像
        ...
        cv2.imwrite(os.path.join(save_dir, '2dbox.png'), img1)
        cv2.imwrite(os.path.join(save_dir, '3dbox.png'), img2)
    else: # 将可视化结果直接可视化
...
def show_lidar_on_image(pc_velo, img, calib, img_width, img_height, save_dir='output'):
    ...
    if save_dir is not None: # 将可视化结果保存为图像
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        cv2.imwrite(os.path.join(save_dir, 'projection.png'), img)
    else: # 将可视化结果直接可视化
        cv2.imshow("projection", img)
    return img


def show_lidar_topview_with_boxes(pc_velo, objects, calib, objects_pred=None, save_dir='output'):
    """ top_view image
    Args:
    ...
    if save_dir is not None: # 将可视化结果保存为图像
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        cv2.imwrite(os.path.join(save_dir, 'bev.png'), top_image)
    else: # 将可视化结果直接可视化
