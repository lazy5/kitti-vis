...
class Calibration(object):
    ...
    def __init__(self, calib_filepath, from_video=False, kitti_format='kitti-object'):
        """ kitti_format 为kitti数据集格式 可选择(kitti-mot, kitti-object) """
        if from_video:
            calibs = self.read_calib_from_video(calib_filepath)
        else:
            calibs = self.read_calib_file(calib_filepath, kitti_format)
    ...
    def read_calib_file(self, filepath, kitti_format='kitti-object'):
        ...
        with open(filepath, "r") as f:
            for line in f.readlines():
                ...
                if len(line) == 0:
                    continue
                if kitti_format == 'kitti-mot':
                    line = line.replace('R_rect', 'R0_rect:')\
                                .replace('Tr_velo_cam', 'Tr_velo_to_cam:')\
                                .replace('Tr_imu_velo', 'Tr_imu_to_velo:')
...
def read_mot_label(label_filename, frame_id):
    lines = [line.rstrip() for line in open(label_filename)]
    lines = [line.split(" ") for line in lines]
    lines = [line[2:] for line in lines if int(line[0]) == frame_id]
    objects = [Object3d(line) for line in lines]
    return objects
